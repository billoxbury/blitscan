---
title: "Dashboard forBirdLife LitScan scraper"
author: "Bill Oxbury"
date: "`r Sys.Date()`"
...
dbfile <- "../data/master.sqlite"

conn <- DBI::dbConnect(RSQLite::SQLite(), dbfile)
df <- tbl(conn, 'links')
df_tx <- df %>% filter(GOTTEXT == 1 & BADLINK == 0 & score > -20.0)
```
...
The
<a href="http://blitscan.uksouth.azurecontainer.io:3838/">BlitScan app</a> is built over a data set of journal articles/research publications:
...
cat("Number of species:", length(species_unique), '\n')
```
...
title_dois <- inner_join(df_dois, df_tx, by = c('doi','doi')) %>%
  ...
  collect()
title_journals <- tbl(conn, 'journals') %>%
  select('Journal') %>%
  collect()
...
title_list <- rbind(title_dois, title_journals, title_oai) %>%
  distinct()
nt <- nrow(title_list)
...
The counts in the table below are the numbers of species (normalised to _SISRecID_ identifier and classified by red-list status) occuring in any title or abstract of the database, versus those occuring in the <a href="http://datazone.birdlife.org/home">BirdLife DataZone</a>.
...
full_join(df_oai, df_tx_oai, by = 'Society') %>%
  arrange(Journal) %>%
...
```
<p>
Note that points beyond today's date usually indicates instances where publication comes later than online announcement. 
