...
df_blit %>%
  ...
  geom_boxplot(colour = "blue",
               fill = "lightblue") +
  scale_y_continuous(trans='log10', breaks=c(1,2,3,5,10,20,30,50,100,200,300,500)) +
...
plt <- df_tx %>% 
  mutate(score = score, language = str_remove(language, '\\|en$|^en\\|')) %>%
  group_by(language) %>%
  ggplot(aes(x = reorder(language, score, max), y = score)) 

plt +
  geom_boxplot(colour = "blue",
               fill = "lightblue") +
  coord_flip() +
  labs(y="Relevance score", x="Language", 
       subtitle="")
...
cat(sprintf("Proportion of records with date parsed: %g%%\n",
            100 * ( 1 - sum(baddate, na.rm = TRUE) / nr ) 
            %>% round(3)
))
```
